...
import com.stolser.javatraining.webproject.controller.ApplicationResources.{
	MSG_USER_EMAIL_DUPLICATION_ERROR,
	MSG_USER_EMAIL_REGEX_ERROR,
	STATUS_CODE_VALIDATION_FAILED,
	USER_EMAIL_PATTERN_REGEX
}
...
object UserEmailValidator extends AbstractValidator {
	...
	override protected def checkParameter(userEmail: String, request: HttpServletRequest): Optional[ValidationResult] = {
		if (!emailMatchesRegex(userEmail)) return Optional.of(regexFailedResult)
		if (emailExistsInDb(userEmail)) return Optional.of(duplicationFailedResult)

...
import com.stolser.javatraining.webproject.model.entity.user.User.Role
...
class MysqlRoleDao(conn: Connection) extends RoleDao {
	...
	override def findRolesByUserName(userName: String): util.Set[User.Role] = {
			...
			withResources(conn.prepareStatement(sqlStatement)) {
				st: PreparedStatement => {
					...
					withResources(st.executeQuery()) {
						rs: ResultSet =>
							val roles = new util.HashSet[User.Role]()
							while (rs.next)
	...
	override def addRole(userId: Long, role: User.Role): Unit = {
